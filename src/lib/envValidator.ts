const REQUIRED_ENV_VARS = ['NEXTAUTH_SECRET', 'NEXTAUTH_URL', 'DATABASE_URL'] as const
...
const OPTIONAL_ENV_VARS = [
  ...
  'LLM_MODEL',
  'NEXT_PUBLIC_APP_URL',
  'XIAOYING_OIDC_CLIENT_ID',
  'XIAOYING_OIDC_CLIENT_SECRET',
] as const
...
export function validateEnv(): EnvValidationResult {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (process.env.DATABASE_URL?.startsWith('file:')) {
      warnings.push(
        'SQLite file database detected in production. Consider using a proper database server.',
      )
    }
...
export function getRequiredEnvVar(name: string): string {
  ...
  if (isInsecureValue(value)) {
    console.warn(
      `[SECURITY WARNING] Environment variable ${name} appears to have an insecure default value`,
    )
  }
  return value
}
