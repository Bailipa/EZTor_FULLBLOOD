...
export async function syncUserWordWithPublic(
  ...
  try {
    ...
    if (sameText(userWord.phonetic, publicWord.phonetic) || isBlank(userWord.phonetic))
      nextData.phonetic = null
    if (sameText(userWord.pos, publicWord.pos) || isBlank(userWord.pos)) nextData.pos = null
    if (sameText(userWord.translation, publicWord.translation) || isBlank(userWord.translation))
      nextData.translation = null
    if (sameText(userWord.example, publicWord.example) || isBlank(userWord.example))
      nextData.example = null
    if (
      sameText(userWord.exampleTranslation, publicWord.exampleTranslation) ||
      isBlank(userWord.exampleTranslation)
    ) {
...
export async function syncAllUserWordsWithPublic(userId: string): Promise<SyncResult> {
  ...
  try {
    ...
    for (const uw of userWords) {
      ...
      if (
        sameText(uw.exampleTranslation, pw.exampleTranslation) ||
        isBlank(uw.exampleTranslation)
      ) {
        if (uw.exampleTranslation !== null) {
          nextData.exampleTranslation = null
          needsUpdate = true
        }
      }
...
export async function checkAndSyncOnQuery(
  ...
  try {
    ...
    const syncIdsByPublicWordId = new Map<
      string,
      { uwIds: string[]; publicWord: (typeof publicWords)[0] }
    >()
