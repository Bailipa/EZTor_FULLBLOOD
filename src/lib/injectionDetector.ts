...
function regexDetect(input: string): { detected: boolean; pattern?: string } {
  ...
  if (
    lowerInput.includes('翻译') &&
    (lowerInput.includes('语气') ||
      lowerInput.includes('口吻') ||
      lowerInput.includes('风格') ||
      lowerInput.includes('方式'))
  ) {
    return { detected: true, pattern: 'translate_with_style' }
  }

  if (
    lowerInput.includes('你是') &&
    (lowerInput.includes('猫') ||
      lowerInput.includes('娘') ||
      lowerInput.includes('角色') ||
      lowerInput.includes('身份'))
  ) {
