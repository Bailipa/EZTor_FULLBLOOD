...
const CSRF_EXEMPT_PATHS = ['/api/auth', '/api/captcha', '/api/analytics', '/api/debug', '/api/downloads/record']
...
function getAllowedOrigins(host: string): string[] {
  return [`http://${host}`, `https://${host}`, process.env.NEXT_PUBLIC_APP_URL].filter(
    Boolean,
  ) as string[]
}
