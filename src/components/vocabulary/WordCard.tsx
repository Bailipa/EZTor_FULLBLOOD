    ...
    >
      <CardContent className="p-5 space-y-3 relative" style={{ minHeight: '200px', contain: 'layout' }}>
        ...
        <div className="flex items-start justify-between">
          ...
          {!isSelectionMode && (
            <AlertDialog open={isDeleting} onOpenChange={(open) => !open && onSetDeletingId(null)}>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    确定要{isGroupView ? "从当前分组中移除" : "从生词本中永久删除"} <span className="font-bold text-gray-900">"{item.word}"</span> 吗？
                    {isGroupView && " (该单词仍会保留在您的总生词本中)"}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>取消</AlertDialogCancel>
                  <AlertDialogAction onClick={() => onDelete(item.id)} className="bg-red-600 hover:bg-red-700">确定移除</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
          )}
