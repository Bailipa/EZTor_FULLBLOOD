...
import { Check, X, Loader2, Volume2, Layers, BookmarkPlus } from "lucide-react";
          ...
          <div className="absolute top-2 right-10 flex gap-2 z-10">
            {currentWord && session?.user && (
              <Button 
                ...
                title="添加到我的生词本"
              >
                {isSaving ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <BookmarkPlus className="w-3 h-3 mr-1" />}
                加入生词本
              </Button>
            )}
          </div>
