      ...
      <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 items-end">
                    <div>
                      ...
                      <Input value={form.quotaRemaining} onChange={(e) => setForm((p) => ({ ...p, quotaRemaining: e.target.value }))} />
                    </div>
                    <div className="flex items-center gap-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>系统会按 priority 顺序选择可用 API（quotaRemaining 大于 0 或为空）</CardDescription>
          </CardHeader>
          <CardContent>
            {error ? (
                    ...
                    {sortedRows.map((r) => (
                      <tr key={r.id} className="border-b hover:bg-muted/30">
                        <td className="py-2 px-2 font-medium">
                          {r.name} {!r.isActive ? <span className="text-xs text-muted-foreground">(禁用)</span> : null}
                        </td>
                        <td className="py-2 px-2 text-muted-foreground whitespace-nowrap">{r.apiKey}</td>
                        <td className="py-2 px-2 text-muted-foreground max-w-[220px] truncate">{r.baseUrl}</td>
                        <td className="py-2 px-2 text-muted-foreground">{r.model}</td>
                        <td className="py-2 px-2 text-right">{r.priority}</td>
                        <td className="py-2 px-2 text-right">{r.quotaRemaining === null ? '∞' : r.quotaRemaining}</td>
                        <td className="py-2 px-2 text-right">{r.quotaUsed}</td>
                        <td className="py-2 px-2 text-muted-foreground whitespace-nowrap">{fmt(r.lastUsedAt)}</td>
                        <td className="py-2 px-2 text-muted-foreground max-w-[220px] truncate" title={r.lastError || ''}>
                          {r.lastError || '-'}
                        </td>
                        <td className="py-2 px-2 text-right whitespace-nowrap">
                          <Button variant="outline" size="sm" onClick={() => editRow(r)} className="mr-2">
                            编辑
                          </Button>
