  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      return NextResponse.json(
        { success: false, error: 'simple words.xlsx not found in the root directory.' },
        { status: 404 },
      )
    }
    ...
    if (stats.size > MAX_FILE_SIZE) {
      return NextResponse.json(
        {
          success: false,
          error: `File too large (${(stats.size / 1024 / 1024).toFixed(1)} MB). Maximum allowed is 10 MB.`,
        },
        { status: 413 },
      )
    }
    ...
    if (rawData.length > MAX_ROWS) {
      return NextResponse.json(
        {
          success: false,
          error: `Too many rows (${rawData.length}). Maximum allowed is ${MAX_ROWS}.`,
        },
        { status: 413 },
      )
    }
