...
export async function GET(req: NextRequest) {
  try {
    ...
    const minQuality = searchParams.get('minQuality')
      ? parseInt(searchParams.get('minQuality')!)
      : undefined
    const maxQuality = searchParams.get('maxQuality')
      ? parseInt(searchParams.get('maxQuality')!)
      : undefined
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        stats: {
          ...
          qualityDistribution: qualityDistribution.map(
            (q: { qualityScore: number; _count: { id: number } }) => ({
    ...
    if (word !== undefined) {
      ...
      if (normalized !== existing.word) {
        return NextResponse.json(
          {
            success: false,
            error: 'Renaming words is not supported. Please create a new word instead.',
          },
          { status: 400 },
        )
      }
