...
export async function POST(req: Request) {
  ...
  try {
    ...
    const sanitizedCustomName = rawCustomName
      ? sanitizeInput(String(rawCustomName).trim(), 100)
      : ''
    ...
    if (targetGroupId) {
      ...
      const errorResponse = {
        success: false,
        error: '必须指定目标分组或设置 createNewGroup 为 true',
      }
      if (isTest) {
        return new Response(JSON.stringify(errorResponse), {
          status: 400,
    ...
    try {
      ...
      for (let i = 0; i < words.length; i += batchSize) {
        ...
        push(
          JSON.stringify({
            progress,
            step: `导入词汇 ${i + 1}-${Math.min(i + batchSize, words.length)}`,
          }),
        )
            ...
            for (const wordData of batch) {
              try {
                ...
                const effectiveTranslation =
                  wordData.translation ?? wordData.publicWord?.translation ?? ''
      ...
      if (tranCode === 'SQLITE_CONSTRAINT') {
        ...
        errorCode = 'TIMEOUT'
        suggestion =
          '系统已自动采用分批处理策略（每批 50 个词汇），如仍超时建议：1) 联系分享者拆分词库 2) 选择词汇数较少的词库 3) 稍后重试'
