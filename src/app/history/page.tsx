...
export default function HistoryPage() {
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        if (isGroupView) {
          const res = await fetch(`/api/review-groups/${groupIdRef.current}/words?wordId=${id}`, {
            method: 'DELETE',
          })
          if (!res.ok) {
  ...
  const handleClearAll = useCallback(async () => {
    ...
    try {
      if (isGroupView) {
        const res = await fetch(`/api/review-groups/${currentViewGroupId}/words?action=clear_all`, {
          method: 'DELETE',
        })
        if (!res.ok) {
          fetchWords(currentViewGroupId)
