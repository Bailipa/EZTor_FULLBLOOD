...
**测试覆盖 (3 个测试文件，55 个测试用例)**：
1. 密钥生成器 (`codeGenerator.ts`) — 17 个测试用例
2. 分享导入 API (`/api/share/import/route.ts`) — 28 个测试用例
3. 分享验证 API (`/api/share/validate/[code]/route.ts`) — 10 个测试用例
...
#### 导入流程
- ✅ 成功导入词汇
- ✅ 当 `skipExisting` 为 true 时跳过已存在的单词但将其链接到目标分组
- ✅ 当单词已存在于目标分组时不创建重复关联
- ✅ 更新分享使用次数
- ✅ 创建导入记录
...
**文件**: [`src/app/api/share/validate/[code]/__tests__/validate.test.ts`](file:///Users/elee987/Downloads/web_compressed/src/app/api/share/validate/[code]/__tests__/validate.test.ts)
...
#### 无效密钥场景
- ✅ 不存在的密钥返回 INVALID_CODE
- ✅ 已撤销分享返回 INACTIVE_SHARE
- ✅ 过期密钥返回 EXPIRED_CODE
- ✅ 达到最大使用次数返回 MAX_USES_REACHED
...
**运行测试**:
```bash
npm test -- src/app/api/share/validate/[code]/__tests__/validate.test.ts
```
...
1. Vitest 配置和依赖安装
2. 密钥生成器测试（17 个测试用例）
3. 分享导入 API 测试（28 个测试用例）
4. 分享验证 API 测试（10 个测试用例）
5. Mock 策略实现
6. 错误处理测试
7. 事务回滚测试
