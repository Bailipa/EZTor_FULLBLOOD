    ...
    await prisma.$transaction(async (tx) => {
      for (let i = 0; i < words.length; i += batchSize) {
        ...
        if (processed % 500 === 0 || processed === words.length) {
          ...
          console.log(
            `    进度：${processed}/${words.length} (${percent}%) - 已导入：${imported}, 跳过：${skipped}, 失败：${failed}`,
          )
        }
      }
    })
...
async function main() {
  ...
  // 确定要导入的词库
  const categoriesToImport = category === 'all' ? Object.keys(VOCABULARY_CONFIG) : [category]
  ...
  if (!fs.existsSync(jsonDir)) {
    console.log(
      '❌ 未找到词汇数据，请先运行：git clone https://github.com/KyleBing/english-vocabulary.git /tmp/english-vocabulary',
    )
    return
  }
