...
function sqliteJSON(sql: string): any[] {
  execSync(`sqlite3 "${BACKUP_PATH}" -json "${sql}" > /tmp/sqlite_export.json`, {
    encoding: 'utf8',
  })
...
async function main() {
  ...
  for (const w of words) {
    try {
      await pg.$executeRawUnsafe(
        ...
        new Date(w.updatedAt || Date.now()),
        null,
        w.sourceType || 'USER', // set publicWordId to null to avoid FK issues
      )
      wordCount++
    } catch (_e: any) {
      wordErrors++
  ...
  for (const c of apiConfigs) {
    ...
    await pg.apiConfig.upsert({
      ...
      create: {
        id: 'global',
        apiKey: c.apiKey || '',
        baseUrl: c.baseUrl || 'https://api.openai.com/v1/chat/completions',
  ...
  try {
    ...
    console.log('  IgnoredWord table may not exist')
  }
  ...
  try {
    ...
    for (const s of shares) {
      await pg.sharedVocabulary.upsert({
        ...
        create: {
          ...
          userId: s.userId,
          shareType: s.shareType || 'GROUP',
          reviewGroupId: s.reviewGroupId,
    ...
    console.log('  SharedVocabulary table may not exist')
  }
  ...
  try {
    ...
    for (const p of providers) {
      await pg.llmApiProvider.upsert({
        ...
        create: {
          ...
          isActive: !!p.isActive,
          quotaRemaining: p.quotaRemaining || null,
          quotaUsed: p.quotaUsed || 0,
          lastUsedAt: p.lastUsedAt ? new Date(p.lastUsedAt) : null,
          lastError: p.lastError || null,
          lastErrorAt: p.lastErrorAt ? new Date(p.lastErrorAt) : null,
    ...
    console.log('  LlmApiProvider table may not exist')
  }
