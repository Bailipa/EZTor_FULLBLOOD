...
async function supplementExamplesForExistingWords(tatoebaMap: Map<string, TatoebaPair[]>) {
  ...
  const wordsWithoutExamples = await prisma.word.findMany({
    where: {
      OR: [{ example: { equals: null } }, { example: { equals: '' } }],
    },
  ...
  for (let i = 0; i < wordsWithoutExamples.length; i += batchSize) {
    ...
    if ((i + batchSize) % 500 === 0 || i + batchSize >= wordsWithoutExamples.length) {
      console.log(
        `  Progress: ${Math.min(i + batchSize, wordsWithoutExamples.length)}/${wordsWithoutExamples.length} (supplemented: ${supplemented})`,
      )
    }
  }
  ...
  for (let i = 0; i < entries.length; i += batchSize) {
    ...
    for (const entry of batch) {
      ...
      if (existingWord) {
        ...
        if (needsUpdate) {
          const example = existingWord.example ? null : pickBestExamples(normalizedWord, tatoebaMap)
          ...
          await prisma.word.update({
            ...
            data: {
              ...
              ...(example
                ? { example: example.example, exampleTranslation: example.exampleTranslation }
                : {}),
            },
          })
...
async function main() {
  ...
  if (!fs.existsSync(ecdictPath)) {
    ...
    console.error(
      'Download from: https://raw.githubusercontent.com/skywind3000/ECDICT/master/ecdict.csv',
    )
    process.exit(1)
  }
