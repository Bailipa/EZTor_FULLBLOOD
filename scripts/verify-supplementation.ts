  ...
  for (const w of sampled) {
    ...
    if (w.exampleTranslation && w.exampleTranslation.trim().length === 0) {
      issues.push({
        ...
        issue: '例句翻译为空',
        currentValue: w.exampleTranslation,
      })
    }
...
async function main() {
  ...
  const report: VerificationReport = {
    ...
    summary: {
      ...
      publicWordTable: {
        ...
        withExamplePercent:
          publicTotal > 0 ? Math.round((publicWithExample / publicTotal) * 100) : 0,
        withPhonetic: publicWithPhonetic,
        withPhoneticPercent:
          publicTotal > 0 ? Math.round((publicWithPhonetic / publicTotal) * 100) : 0,
        ...
        fullyCompletePercent:
          publicTotal > 0 ? Math.round((publicFullyComplete / publicTotal) * 100) : 0,
      },
    },
    qualityCheck: {
      examplesSampled: 100,
  ...
  console.log(
    `  有例句:     ${report.summary.wordTable.withExample} (${report.summary.wordTable.withExamplePercent}%)`,
  )
  console.log(
    `  有音标:     ${report.summary.wordTable.withPhonetic} (${report.summary.wordTable.withPhoneticPercent}%)`,
  )
  console.log(
    `  有词性:     ${report.summary.wordTable.withPos} (${report.summary.wordTable.withPosPercent}%)`,
  )
  console.log(
    `  完整词条:   ${report.summary.wordTable.fullyComplete} (${report.summary.wordTable.fullyCompletePercent}%)`,
  )

  console.log('\n📋 PublicWord 表:')
  console.log(`  总计:       ${report.summary.publicWordTable.total}`)
  console.log(
    `  有例句:     ${report.summary.publicWordTable.withExample} (${report.summary.publicWordTable.withExamplePercent}%)`,
  )
  console.log(
    `  有音标:     ${report.summary.publicWordTable.withPhonetic} (${report.summary.publicWordTable.withPhoneticPercent}%)`,
  )
  console.log(
    `  有词性:     ${report.summary.publicWordTable.withPos} (${report.summary.publicWordTable.withPosPercent}%)`,
  )
  console.log(
    `  完整词条:   ${report.summary.publicWordTable.fullyComplete} (${report.summary.publicWordTable.fullyCompletePercent}%)`,
  )
  ...
  if (report.qualityCheck.issues.length > 0) {
    ...
    for (const issue of report.qualityCheck.issues.slice(0, 10)) {
      console.log(
        `    - [${issue.word}] ${issue.field}: ${issue.issue} (当前值: "${(issue.currentValue || '').substring(0, 60)}")`,
      )
    }
  }
