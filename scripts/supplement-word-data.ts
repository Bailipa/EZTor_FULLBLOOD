...
async function supplementWords(
  ...
  const stats: SupplementStats = {
    ...
    publicWordTable: {
      total: 0,
      exampleSupplemented: 0,
      phoneticSupplemented: 0,
      posSupplemented: 0,
    },
  }
...
async function printDatabaseSnapshot() {
  ...
  console.log(
    `    有例句: ${wordsWithExamples} (${totalWords > 0 ? Math.round((wordsWithExamples / totalWords) * 100) : 0}%)`,
  )
  console.log(
    `    有音标: ${wordsWithPhonetic} (${totalWords > 0 ? Math.round((wordsWithPhonetic / totalWords) * 100) : 0}%)`,
  )
  console.log(
    `    有词性: ${wordsWithPos} (${totalWords > 0 ? Math.round((wordsWithPos / totalWords) * 100) : 0}%)`,
  )
  console.log('')
  console.log(`  PublicWord 总计:  ${totalPublic}`)
  console.log(
    `    有例句: ${publicWithExamples} (${totalPublic > 0 ? Math.round((publicWithExamples / totalPublic) * 100) : 0}%)`,
  )
  console.log(
    `    有音标: ${publicWithPhonetic} (${totalPublic > 0 ? Math.round((publicWithPhonetic / totalPublic) * 100) : 0}%)`,
  )
  console.log(
    `    有词性: ${publicWithPos} (${totalPublic > 0 ? Math.round((publicWithPos / totalPublic) * 100) : 0}%)`,
  )
  console.log('─'.repeat(50))
}
...
async function main() {
  ...
  console.log(
    `   目标: ${wordOnly ? '仅 Word 表' : publicOnly ? '仅 PublicWord 表' : 'Word + PublicWord 表'}\n`,
  )
