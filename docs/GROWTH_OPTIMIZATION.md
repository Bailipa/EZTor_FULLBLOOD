...
1. 在 `FullscreenFlashcard` 中，当引导步骤为 1 时：
   - 显示 Tooltip 气泡，指向"显示答案"按钮
   - 文案："点击'显示答案'查看单词释义"
   - 答案显示后，指向"认识"/"不认识"按钮
   - 文案："点击'认识'表示你知道这个词，点击'不认识'表示你不知道"

2. 用户点击"认识"/"不认识"按钮后：
   - 执行保存逻辑（存入生词本）
   - 推进步骤到 2
   - 使用 `router.push('/dictation')` 跳转到默写页
...
1. 在默写页面，当引导步骤为 2 时：
   - 显示 OnboardingTooltip，覆盖在"开始测试"按钮上方
   - 文案："来试试默写吧！点击'知道了'开始默写，这次只默写 1 个单词"
   - 用户点击"知道了"后，自动开始默写 1 个单词

2. 默写完成后：
   - 显示完成弹窗
   - 文案："默写完成！你已经掌握了默写功能。接下来去看看你的生词本吧！"
   - 用户点击"查看生词本"后，使用 `router.push('/history')` 跳转

3. 默写单词来源：
   - 从公共词库随机 1 个

**Verification**:
- 引导步骤 2 时，默写页面显示 Tooltip 覆盖在"开始测试"按钮上方
- 点击"知道了"后自动开始默写 1 个单词
- 默写完成后显示完成弹窗，点击"查看生词本"跳转到生词本页
...
### Step 2.6 — 引导步骤 5：Chat 功能介绍

**Problem**: 用户不知道有反馈聊天功能。
...
1. 在 `MobileNavBar` 中，当引导步骤为 5 时：
   - 高亮"反馈"入口（添加特殊样式）
   - 显示 OnboardingTooltip 指向"反馈"按钮
   - 文案："有问题或建议？点击这里与开发者交流"

2. 用户点击"知道了"后：
   - 使用 `router.push('/chat')` 导航到 `/chat` 页面

3. 在 `/chat` 页面加载时：
   - 检测到 `isActive && currentStep === 5`
   - 自动调用 `nextStep()` 推进步骤到 6

**Verification**:
- 引导步骤 5 时，导航栏"反馈"入口高亮
- 显示 Tooltip 引导
- 点击"知道了"后跳转到 Chat 页面
- 到达 Chat 页面后自动推进步骤到 6
- 用户返回首页时步骤 6 出现

---

### Step 2.7 — 引导步骤 6：功能探索提示
...
2. 同时显示 OnboardingTooltip 指向"完成引导"按钮
   - 文案："点击'完成引导'开始你的学习之旅"
...
**Verification**:
- 引导步骤 6 时，首页底部显示浮动卡片
- 显示 OnboardingTooltip 指向"完成引导"按钮
- 点击"完成引导"后，引导结束
- 下次登录不再显示引导
...
| 2 | 2.1 引导状态管理 | 🔴 高 | 0.1 | ✅ 已完成 |
| 2 | 2.2 闪卡使用提示 | 🟡 中 | 1.1, 2.1 | ✅ 已完成 |
| 2 | 2.3 初次默写体验 | 🟡 中 | 2.1 | ✅ 已完成 |
| 2 | 2.4 生词本展示 | 🟡 中 | 2.1 | ✅ 已完成 |
| 2 | 2.5 词库导入介绍 | 🟢 低 | 2.1 | ✅ 已完成 |
| 2 | 2.6 Chat 功能介绍 | 🟡 中 | 2.1 | ✅ 已完成 |
| 2 | 2.7 功能探索提示 | 🟢 低 | 2.1 | ✅ 已完成 |
...
### Phase 2 验证
- [x] 新用户登录后显示引导
- [x] 引导步骤 1：闪卡使用提示（交互式指引，箭头指向目标按钮）
- [x] 引导步骤 2：初次默写 1 个单词（Tooltip 覆盖在"开始测试"按钮上方，点击"知道了"后开始）
- [x] 引导步骤 3：生词本展示（底部浮动卡片，点击"知道了"推进）
- [x] 引导步骤 4：词库导入介绍（Tooltip 指向导入按钮，点击"知道了"跳转首页）
- [x] 引导步骤 5：Chat 功能介绍（Tooltip 指向反馈按钮，点击"知道了"跳转 /chat，自动推进步骤）
- [x] 引导步骤 6：功能探索提示（底部卡片 + OnboardingTooltip，点击"完成引导"结束）
- [x] 引导完成后不再显示
- [x] 桌面端跳过步骤 5（反馈聊天介绍）
- [x] 使用 router.push 替代 window.location.href 进行客户端导航
- [x] OnboardingProvider 保存步骤到 localStorage，页面跳转后恢复
